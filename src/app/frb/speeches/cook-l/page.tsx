...
async function getRssFeed() {
  const response = await fetch('https://www.federalreserve.gov/feeds/s_t_cook.xml');
  const xml = await response.text();
  const result = await parseStringPromise(xml);
  return result.rss.channel[0].item as RssItem[];
}

export default async function CookLisa() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <h1 className="text-center text-xl tracking-widest text-[#355e93] uppercase">Governor Lisa D. Cook</h1>
