...
async function getRssFeed() {
  const response = await fetch('https://www.federalreserve.gov/feeds/m_w_Bowman.xml');
  const xml = await response.text();
  const result = await parseStringPromise(xml);
  return result.rss.channel[0].item as RssItem[];
}

export default async function BowmanMichelle() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <h1 className="text-center text-xl tracking-widest text-gray-700 uppercase">Vice Chair for Supervision Michelle W. Bowman</h1>
