...
async function getRssFeed() {
  const response = await fetch('https://www.federalreserve.gov/feeds/s_t_jefferson.xml');
  const xml = await response.text();
  const result = await parseStringPromise(xml);
  return result.rss.channel[0].item as RssItem[];
}

export default async function JeffersonPhilip() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <h1 className="text-center text-xl tracking-widest text-gray-700 uppercase">Vice Chair Philip N. Jefferson</h1>
